...
public partial class  Ruch_Slime:Node{
	...
	public Vector2 przesuwanieSie(Vector2 velocity, double delta){
		if(slime.uderzony){
			...
			return new Vector2(Speed * kierunek * (float) delta * 30 * slime.moc.X, -150 * slime.moc.Y);
		}
...
public partial class Zdrowie_Slime : Node{
	...
	public void wejscieObiektu(Node area){
		if(area.IsInGroup("atakiRycerza")){
			if(area.Name == "Odpychanie"){
				healthPoints -= (int) Math.Floor((double) Global.obrazenia / 2);
				slime.moc = new Vector2(1.85f, 1.35f);
			}
			else if(area.Name == "ZakresMiecza"){
				healthPoints -= Global.obrazenia;
				slime.moc = new Vector2(1, 1);
			}
			umieranie();
			slime.uderzony = true;
			if(healthPoints <= 0){
